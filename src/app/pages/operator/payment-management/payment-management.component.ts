...
export class PaymentManagementComponent {
  ...
  private readonly code = this.route.snapshot.queryParamMap.get('reservation') || 'RES-1843';
  ...
  isSettled = computed(() => this.reservation().payment === 'Pagado' || parseCurrency(this.reservation().balance) === 0);
  ...
  paymentHeading = computed(() => (this.isSettled() ? 'Pago registrado' : 'Registra el pago de esta reserva'));

  private resolveInitial(): OperatorReservation {
    const found = this.reservationService.getReservation(this.code);
    if (found) return { ...found };
    return {
      code: this.code, customer: 'Paula', email: '', service: '', date: '', travelers: 0, companions: '',
      status: 'Pendiente de pago', statusClass: 'is-pending', projected: '$2.078.400', discount: '$0',
      final: '$2.078.400', paid: '$0', balance: '$2.078.400', payment: 'Sin pago', method: 'Efectivo', execution: '', action: '', href: '',
    };
  }
  ...
  onSubmit(event: Event): void {
    ...
    if (!this.hasSupportedMethod()) return;
